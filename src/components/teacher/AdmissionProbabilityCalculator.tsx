                ...
                <div className="px-4 pb-5 text-[13px] leading-relaxed text-slate-600 border-t border-slate-100 pt-4 space-y-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <StepRow n={3} title="이 학과만의 충원 패턴 반영">
                      여기서부터는 이 학과 자신의 최근 실제 등록 인원(모집 인원과 추가 합격 인원을 더한
                      값)만 사용해요. 정원이 넉넉할수록 추가 합격이 더 많이 나는 경향이 실제 데이터로
                      확인돼서, 그 경향을 반영해 등록이 끝날 때까지 성적이 얼마나 더 밀릴 수 있는지를
                      계산해요.
                    </StepRow>
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    ...
                    <div className="space-y-2.5">
                      <p>
                        <strong className="text-slate-800">국소가중회귀(커널 기반 유사도)</strong> — 학과를
                        몇 가지 수치(컷의 평균 수준, 최근 추세, 정원 증감, 정규화한 경쟁률)로 이루어진
                        벡터로 표현하고, 목표 학과와의 거리가 가까울수록 큰 가중치를 부여하는 가우시안
                        커널 함수를 사용해요. 통계학에서 국소가중회귀(local weighted regression) 또는
                        Nadaraya–Watson 커널 회귀라 불리는 방식으로, 전체 평균을 내는 대신 &ldquo;지금
                        상황과 비슷한 사례일수록 더 많이 반영&rdquo;하는 방법이에요.
                      </p>
                      <p>
                        <strong className="text-slate-800">가중중앙값(로버스트 통계량)</strong> — 50%·70%컷
                        예측에는 가중평균이 아니라 가중중앙값을 사용해요. 입결 데이터처럼 소수의 극단적
                        사례(미충원, 대규모 추가합격 등)가 꼬리를 만드는 비대칭 분포에서는, 평균이 그
                        극단치에 쉽게 끌려가는 반면 중앙값은 이런 이상치에 상대적으로 영향을 덜 받는
                        (robust) 성질이 있어요.
                      </p>
                      <p>
                        <strong className="text-slate-800">로그선형회귀(등록 마지노선 추정)</strong> —
                        합격선(등록 마지노선)이 50%·70%컷보다 얼마나 더 밀리는지는, 그 학과의 최근 실제
                        등록 인원 규모와 로그선형 관계를 갖는 것으로 실측·추정했어요. 정원이 클수록
                        추가합격 여력이 상대적으로 커지는 경향을 반영한 관계식이에요.
                      </p>
                      <p>
                        <strong className="text-slate-800">몬테카를로 시뮬레이션(붓스트랩 재표집)</strong> —
                        등록 마지노선을 하나의 확정값으로 두지 않고, 확률분포로 취급해요. 국소가중회귀에서
                        구한 가중치를 확률처럼 사용해 과거 실제 컷 값들을 반복 복원추출(bootstrap
                        resampling)하고, 여기에 로그선형회귀가 갖는 추정 오차를 정규분포 형태의 노이즈로
                        더해, 수천 번의 가상 시나리오를 만들어요. 최종 합격확률은 그 시나리오들 중 실제로
                        합격 조건을 만족하는 비율을 그대로 세어서 구해요 — 로지스틱 함수 같은 별도의 확률
                        변환식을 쓰지 않아요.
                      </p>
                      <p>
                        <strong className="text-slate-800">유효표본수 기반 신뢰구간</strong> — 확률을 하나의
                        숫자 대신 범위로 보여주는 이유는, 가중치를 사용한 추정값의 불확실성을 통계학의
                        유효표본수(effective sample size, Kish&apos;s approximation) 개념으로
                        정량화했기 때문이에요. 참고할 수 있는 비슷한 사례가 많을수록(유효표본수가
                        클수록) 범위가 좁아지고, 적을수록 넓어져요.
                      </p>
                    </div>
                  </div>
                </div>
